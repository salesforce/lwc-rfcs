...
To avoid typos (e.g. using `lwc:else` when you mean `elseif:true`), `lwc:else` must _not_ have an attribute value. Any attribute value is treated as a compile-time error:
...
1. The property getters are only accessed once per instance of an `if:` or `ifelse:`.
