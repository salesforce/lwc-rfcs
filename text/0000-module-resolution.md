...
This type of resolution allows to point to to a folder that contain LWC modules.
Its important to note that within that folder the structure must be very specific, it must contain a namespace folder, then a folder per module bundle (moduleName) adnd them the moduleEntry file that matches the name of the bundle (the entry must be of type .css, .html, .js or .ts)
